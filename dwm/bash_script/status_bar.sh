...
while true; do
    xsetroot -name "$(printf '%s\x01%s\x01\x02%s\x02\x03%s\x03\x04%s\x04' "$(cat $MEM)  $(cat $INTERNET) $(cat $WIFI)  $(cat $CPU)  " "$(cat $MODE)  " "$(cat $MIC)  " "$(cat $MUTE) $(cat $VOLUME)  " "$(cat $BAT)  $DATE $(date +"%H:%M:%S")")"
    sleep 1
done
